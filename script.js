...
function moveDogs() {
  ...
  const lTargetX = clamp(mx - 40, DOG_W, window.innerWidth);
  const lTargetY = clamp(my - 25, DOG_H, window.innerHeight - 70);
  lunaX += (lTargetX - lunaX) * 0.07;
  lunaY += (lTargetY - lunaY) * 0.07;


  const aTargetX = clamp(mx - 70, DOG_W, window.innerWidth);
  const aTargetY = clamp(my - 55, DOG_H, window.innerHeight - 70);
