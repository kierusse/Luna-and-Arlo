...
function moveDogs() {
  ...
  const lTargetX = clamp(mx, DOG_W, window.innerWidth  - DOG_W);
  const lTargetY = clamp(my, DOG_H, window.innerHeight - DOG_H - 70);
  lunaX += (lTargetX - lunaX) * 0.07;
  lunaY += (lTargetY - lunaY) * 0.07;


  const aTargetX = clamp(mx, DOG_W, window.innerWidth  - DOG_W);
  const aTargetY = clamp(my, DOG_H, window.innerHeight - DOG_H - 70);
